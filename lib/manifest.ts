...
const streamToString = async (stream: unknown): Promise<string> => {
  ...
  if (
    typeof (stream as { transformToString?: () => Promise<string> }).transformToString ===
    "function"
  ) {
...
export const getManifest = async (): Promise<ManifestImage[]> => {
  try {
...
export const putManifest = async (manifest: ManifestImage[]): Promise<void> => {
  await r2Client.send(
